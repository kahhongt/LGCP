...
initial_scalar = 1.5
initial_param = np.array([initial_scalar, initial_scalar, initial_scalar, initial_scalar, initial_scalar])

start_gp_opt = time.clock()

# ChangeParam
ker_space = 'matern1'
ker_time = 'squared_exponential'
...
param_sol = scopt.minimize(fun=gp_likelihood_3d, args=args_param, x0=initial_param,
                           method='Nelder-Mead',
                           options={'xatol': 10, 'fatol': 500, 'disp': True, 'maxfev': 1000})
...
cov_auto = product_kernel_3d(sigma_optimal, length_space_optimal, length_time_optimal,
                             xy_vox, t_vox, ker_space, ker_time)  # Basic Covariance Matrix
...
histo.scatter(x_vox, y_vox, t_vox, c=k_vox, cmap='afmhot', s=5, marker='x')
histo.set_xlabel('UTM Horizontal Coordinate')
histo.set_ylabel('UTM Vertical Coordinate')
histo.set_zlabel('Year')

histo_exclude_fig = plt.figure()
histo_exclude = histo_exclude_fig.add_subplot(111, projection='3d')
histo_exclude.scatter(x_vox_plot, y_vox_plot, t_vox_plot, c=k_vox_plot, cmap='afmhot', s=5, marker='x')
histo_exclude.set_xlabel('UTM Horizontal Coordinate')
